def save_expectations_config(
        ...
        """Writes ``_expectation_config`` to a JSON file.
           ...
           Args:
               filepath (string): \
                   The location and expected to write the JSON config file to.
               ...
               discard_result_format_kwargs (boolean): \
                   If True, the :ref:`result_format` attribute for each expectation is not written to the dropped config file. \
               ...
               suppress_warnings (boolean): \
                  It True, all warnings raised by time Expectations, as a result of dropped expectations, are \
                  suppressed.

        """
        if filepath is None:
            filepath = self.data_asset_type + "_expectations.json"

        self.save_expectation_suite(
            filepath=filepath,
            discard_failed_expectations=discard_failed_expectations,
